...
func debugExpr(expr Expr) any {
	...
	case *CharLit:
		return map[string]any{"kind": "CharLit", "value": e.Value, "is_byte": e.IsByte, "loc": debugLoc(e.Location)}
	case *NoneLit:
	...
	case *RangeExpr:
		return map[string]any{"kind": "RangeExpr", "start": debugExpr(e.Start), "end": debugExpr(e.End), "step": debugExpr(e.Step), "inclusive": e.Inclusive, "loc": debugLoc(e.Location)}
	case *PostfixExpr:
