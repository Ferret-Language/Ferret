...
`)
	...
	if result.Entry == nil || result.Entry.Phase != phase.PhaseOwnershipAnalyzed {
		t.Fatalf("expected ownership-analyzed entry, got %#v", result.Entry)
	}
...
func TestTypecheckerConsumesOwningReceiver(t *testing.T) {
	root := t.TempDir()
	mustWriteType(t, filepath.Join(root, "main.ferr"), `
type Conn struct {}
...
func TestTypecheckerFreezesBorrowedOwnerWithinBlock(t *testing.T) {
	root := t.TempDir()
	mustWriteType(t, filepath.Join(root, "main.ferr"), `
type Conn struct {}
...
`)
	...
	result := compilerapi.New(root, ".ferr", diagnostics.NewBag()).ParseEntry(filepath.Join(root, "main.ferr"))
	if !result.Diagnostics.HasErrors() {
		t.Fatal("expected borrow conflict diagnostic")
	}
	found := false
	for _, diag := range result.Diagnostics.Diagnostics() {
...
func TestTypecheckerReleasesBorrowAtEndOfInnerBlock(t *testing.T) {
	root := t.TempDir()
	mustWriteType(t, filepath.Join(root, "main.ferr"), `
type Conn struct {}
...
func TestTypecheckerRejectsReturnedBorrow(t *testing.T) {
	root := t.TempDir()
	mustWriteType(t, filepath.Join(root, "main.ferr"), `
type Conn struct {}
...
`)
	...
	result := compilerapi.New(root, ".ferr", diagnostics.NewBag()).ParseEntry(filepath.Join(root, "main.ferr"))
	if !result.Diagnostics.HasErrors() {
		t.Fatal("expected borrow escape diagnostic")
	}
	found := false
	for _, diag := range result.Diagnostics.Diagnostics() {
...
func TestTypecheckerRejectsReturnedBorrowBinding(t *testing.T) {
	root := t.TempDir()
	mustWriteType(t, filepath.Join(root, "main.ferr"), `
type Conn struct {}
