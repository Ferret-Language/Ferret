...
func TestTypecheckerInfersStringLiteralAsStrInsideUntypedTuple(t *testing.T) {
	root := t.TempDir()
	mustWriteType(t, filepath.Join(root, "main.fer"), `
fn main() -> void {
    print((1, "hi", 3))
...
`)
	...
	if _, ok := tupleType.Elems[1].(*typeinfo.StringType); !ok {
		t.Fatalf("expected second tuple element str, got %#v", tupleType.Elems[1])
	}
	if !typeinfo.IsBuiltinNamed(tupleType.Elems[2], "i32") {
		t.Fatalf("expected third tuple element i32, got %#v", tupleType.Elems[2])
	}
}
