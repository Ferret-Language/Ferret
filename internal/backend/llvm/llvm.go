...
func LowerProgram(units []*backend.Unit) (string, error) {
	...
	declLines = append([]string{
		"declare void @ferret__panic(ptr, i64)",
		"declare void @ferret__interface_panic(ptr, i64, ptr, i64)",
	}, declLines...)
	...
	for _, unit := range units {
		...
		for _, fn := range unit.Module.Functions {
			...
			// Flush any deferred string constants accumulated by a previous
			// function in this module (before the next function that may use them).
		...
		// Flush any deferred constants emitted by the last function.
		if state.deferredB.Len() > 0 {
			b.WriteString(state.deferredB.String())
			state.deferredB.Reset()
		}
...
func (*lowerer) LowerModule(unit *backend.Unit) (*backend.Artifact, error) {
	...
	fmt.Fprintf(&b, "; generated by ferret llvm backend\n\n")
	// Always declare Ferret runtime functions used by compiler-emitted code.
	b.WriteString("declare void @ferret__panic(ptr, i64)\n")
	b.WriteString("declare void @ferret__interface_panic(ptr, i64, ptr, i64)\n\n")
...
// followed by unreachable.  ferret__panic is declared noreturn so LLVM knows
...
func lowerPanicTerm(state *moduleState, t *midmir.PanicTerm) (string, error) {
	...
	case *midmir.StringValue:
		...
		return fmt.Sprintf("call void @ferret__panic(ptr @%s, i64 %d)\nunreachable", sym, len(v.Value)), nil
	default:
