...
func LowerProgram(units []*backend.Unit, includeDebug bool) (string, error) {
	...
	var ctorEntries []globalCtorEntry
	...
	for i, unit := range units {
		...
		initName, err := emitGlobals(&b, state, unit.Module.Globals)
		if err != nil {
			return "", fmt.Errorf("lower program globals [%s]: %w", unit.Module.ImportPath, err)
		}
		if initName != "" {
			ctorEntries = append(ctorEntries, globalCtorEntry{Name: initName, Priority: 100 + i})
		}
	...
	if len(ctorEntries) > 0 {
		b.WriteByte('\n')
		emitGlobalCtorTable(&b, ctorEntries)
	}
...
func (*lowerer) LowerModule(unit *backend.Unit) (*backend.Artifact, error) {
	...
	if initName != "" {
		b.WriteByte('\n')
		emitGlobalCtorTable(&b, []globalCtorEntry{{Name: initName, Priority: 100}})
	}
...
func emitGlobalCtorTable(b *strings.Builder, entries []globalCtorEntry) {
	...
	for _, entry := range entries {
		...
		parts = append(parts, fmt.Sprintf("{ i32, ptr, ptr } { i32 %d, ptr @%s, ptr null }", entry.Priority, entry.Name))
	}
	if len(parts) == 0 {
		return
	}
...
func emitGlobals(b *strings.Builder, state *moduleState, globals []*mir.Global) (string, error) {
	wrote := 0
	var initSpecs []globalInitSpec
	for _, g := range globals {
		if g == nil {
			continue
		}
		definition, init, err := lowerGlobal(state, g)
		if err != nil {
			return "", err
		}
		if init != nil {
			initSpecs = append(initSpecs, *init)
		}
		if state.deferredB.Len() > 0 {
			if wrote > 0 {
				b.WriteByte('\n')
			}
			text := state.deferredB.String()
			b.WriteString(text)
	...
	return emitGlobalInitFunction(b, state, initSpecs)
}

func emitGlobalInitFunction(b *strings.Builder, state *moduleState, initSpecs []globalInitSpec) (string, error) {
	if b == nil || state == nil || len(initSpecs) == 0 {
		return "", nil
	}
	name := becommon.SanitizeIdent(state.modulePrefix + "__global_init")
	var fnB strings.Builder
	fmt.Fprintf(&fnB, "define void @%s() {\nentry:\n", name)
	for _, init := range initSpecs {
		lines, err := lowerGlobalRuntimeInit(state, &init)
		if err != nil {
			return "", fmt.Errorf("global %s: %w", init.Name, err)
		}
		for _, line := range lines {
			fmt.Fprintf(&fnB, "  %s\n", line)
		}
	}
	fnB.WriteString("  ret void\n}\n")
	if state.deferredB.Len() > 0 {
		text := state.deferredB.String()
		b.WriteString(text)
		if !strings.HasSuffix(text, "\n") {
...
func lowerGlobalRuntimeInit(state *moduleState, init *globalInitSpec) ([]string, error) {
	if state == nil || init == nil || init.Value == nil {
		return nil, nil
	}
	state.pendingLines = nil
	prevGlobalInit := state.inGlobalInit
	state.inGlobalInit = true
	defer func() {
		state.inGlobalInit = prevGlobalInit
	}()
	if requiresAggregateAssign(state, init.Type) {
		size, align, err := backend.AggregateSizeAlign(aggregateLayoutContext(state), init.Type)
		if err != nil {
			return nil, err
		}
		assign, err := lowerAggregateAssign(state, &aggregateLocal{
			Name:    init.Name,
			Type:    init.Type,
			Size:    size,
			Align:   align,
		...
		lines := append([]string(nil), state.pendingLines...)
		state.pendingLines = nil
		for _, line := range strings.Split(assign, "\n") {
			if line != "" {
	...
	lowered, err := lowerValue(state, init.Value)
	if err != nil {
		return nil, err
	}
	irType, err := llvmBaseType(init.Type)
	if err != nil {
		return nil, err
	}
	lines := append([]string(nil), state.pendingLines...)
	state.pendingLines = nil
	lines = append(lines, fmt.Sprintf("store %s %s, ptr @%s", irType, lowered, init.Symbol))
	return lines, nil
}

func lowerTempStorage(state *moduleState, label, irType string, align int64, persistent bool) ([]string, string) {
	if align < 1 {
		align = 1
	}
	if persistent && state != nil && state.inGlobalInit {
		sym := nextPrivateGlobalSymbol(state, label)
		fmt.Fprintf(state.deferredB, "@%s = private global %s zeroinitializer, align %d\n", sym, irType, align)
		return nil, "@" + sym
	}
	tmp := freshTemp(state, label)
	return []string{fmt.Sprintf("%s = alloca %s, align %d", tmp, irType, align)}, tmp
}

func nextPrivateGlobalSymbol(state *moduleState, prefix string) string {
	state.nextPrivateGlobal++
	return fmt.Sprintf("__%s%d_%s", becommon.SanitizeIdent(prefix), state.nextPrivateGlobal, becommon.SanitizeIdent(state.modulePrefix))
}

func lowerGlobalTypeName(state *moduleState, typ typeinfo.Type) (string, error) {
	if irType, err := llvmBaseType(typ); err == nil && !requiresAggregateAssign(state, typ) {
		return irType, nil
	}
	if _, ok := backend.UnwrapNamed(typ).(*typeinfo.ArrayType); ok {
	...
	return llvmABITypeName(state, typ)
}

func requiresAggregateAssign(state *moduleState, typ typeinfo.Type) bool {
	if typ == nil {
		return false
	}
...
func lowerGlobal(state *moduleState, g *mir.Global) (string, *globalInitSpec, error) {
	if g.Init == nil {
		return "", nil, nil
	}
	name := llvmSymbol(state, []string{g.Name})
	if line, ok, err := lowerGlobalConstant(state, name, g); err != nil {
		return "", nil, fmt.Errorf("global %s: %w", g.Name, err)
	} else if ok {
		return line, nil, nil
	}
	typeName, err := lowerGlobalTypeName(state, g.Type)
	if err != nil {
		return "", nil, fmt.Errorf("global %s: %w", g.Name, err)
	}
	return fmt.Sprintf("@%s = global %s zeroinitializer", name, typeName), &globalInitSpec{
		Name:   g.Name,
		Symbol: name,
		Type:   g.Type,
		Value:  g.Init,
...
func lowerGlobalConstant(state *moduleState, name string, g *mir.Global) (string, bool, error) {
	if g == nil || g.Init == nil {
		return "", true, nil
	}
	...
	case specialAggregateInterface:
		...
		case *mir.NameValue, *mir.NumberValue, *mir.BoolValue:
		default:
			return "", false, nil
		}
		line, err := lowerGlobalInterface(state, name, g.Type, init)
		if err != nil {
			return "", false, err
		}
	...
	case specialAggregateUnion:
		...
		return fmt.Sprintf("@%s = global [%d x i8] %s", name, size, body), true, nil
	}
	switch v := g.Init.(type) {
	case *mir.CompositeValue:
		if _, ok := backend.ResolveMapType(g.Type); ok {
			return "", false, nil
		}
		body, ok, err := lowerGlobalComposite(state, g.Type, v)
		if err != nil || !ok {
			return "", ok, err
		}
		...
		return fmt.Sprintf("@%s = global %s { %s }", name, typeName, body), true, nil
	}
	lit, ok, err := lowerGlobalValue(state, g.Type, g.Init)
	if err != nil || !ok {
		return "", ok, err
	}
	...
	return fmt.Sprintf("@%s = global %s %s", name, irType, lit), true, nil
}
...
func lowerAggregateAssign(state *moduleState, agg *aggregateLocal, value mir.Value) (string, error) {
	...
	case *mir.StringValue:
		lines, err := lowerAggregateValueToAddr(state, llvmLocalName(agg.PtrName), agg.Type, v)
		if err != nil {
			return "", err
		}
		return strings.Join(lines, "\n"), nil
...
func lowerUnionAssign(state *moduleState, agg *aggregateLocal, value mir.Value) (string, error) {
	...
	if comp, ok := value.(*mir.CompositeValue); ok {
		lines, dst, err := lowerUnionMemberDestination(state, agg, comp.Type())
		if err != nil {
			return "", err
		}
		payloadLines, err := lowerAggregateValueToAddr(state, dst, comp.Type(), comp)
		if err != nil {
			return "", err
		}
	...
	if isAggregateType(state, value.Type()) {
		...
		lines, dst, err := lowerUnionMemberDestination(state, agg, value.Type())
		if err != nil {
			return "", err
		}
...
func lowerUnionMemberDestination(state *moduleState, agg *aggregateLocal, valueType typeinfo.Type) ([]string, string, error) {
	info, err := llvmUnionLayoutInfo(state, agg.Type)
	if err != nil {
		return nil, "", err
	}
	memberIndex := 1
	if _, ok := backend.UnwrapNamed(agg.Type).(*typeinfo.OptionalType); !ok {
		memberIndex, err = llvmUnionMemberIndex(info.Members, valueType)
		if err != nil {
			return nil, "", err
		}
	}
...
func emitStringConstant(state *moduleState, s string) string {
	sym := nextPrivateGlobalSymbol(state, "str")
...
func lowerAggregateCompositeAssign(state *moduleState, agg *aggregateLocal, comp *mir.CompositeValue) (string, error) {
	...
	if arrType, ok := agg.Type.(*typeinfo.ArrayType); ok {
		...
		for i, item := range comp.Items {
			...
			if requiresAggregateAssign(state, arrType.Inner) {
				elemLines, err := lowerAggregateAssignToAddress(state, addr, arrType.Inner, item.Value)
				if err != nil {
					return "", err
				}
	...
	if tupleType, ok := backend.UnwrapNamed(agg.Type).(*typeinfo.TupleType); ok {
		...
		for i, item := range comp.Items {
			...
			if requiresAggregateAssign(state, entry.Type) {
				valueLines, err := lowerAggregateAssignToAddress(state, addr, entry.Type, item.Value)
				if err != nil {
					return "", err
				}
				lines = append(lines, valueLines...)
	...
	for _, field := range structLayout.Fields {
		...
		if requiresAggregateAssign(state, field.Type) {
			valueLines, err := lowerAggregateAssignToAddress(state, addr, field.Type, val)
			if err != nil {
				return "", err
			}
			lines = append(lines, valueLines...)
...
func lowerAggregateAssignToAddress(state *moduleState, addr string, typ typeinfo.Type, value mir.Value) ([]string, error) {
	...
	assign, err := lowerAggregateAssign(state, &aggregateLocal{
		...
		Size:    size,
		Align:   align,
		PtrName: strings.TrimPrefix(addr, "%"),
	}, value)
	if err != nil {
		return nil, err
	}
...
func lowerAggregateValueToAddr(state *moduleState, addr string, typ typeinfo.Type, value mir.Value) ([]string, error) {
	if sliceType, ok := typ.(*typeinfo.SliceType); ok {
		...
		if elems == nil {
			...
			bufLines, buf := lowerTempStorage(state, "slice_lit_buf", fmt.Sprintf("[%d x i8]", allocBytes), elemAlign, true)
...
func lowerInterfaceConcretePointer(state *moduleState, value mir.Value, concreteType typeinfo.Type) ([]string, string, error) {
	...
	if isAggregateType(state, concreteType) {
		...
		lines, tmp := lowerTempStorage(state, "iface_data", typeName, align, true)
		agg := &aggregateLocal{
	...
	lines, tmp := lowerTempStorage(state, "iface_data", irType, align, true)
	lines = append(lines,
...
func lowerValue(state *moduleState, value mir.Value) (string, error) {
	...
	case *mir.StringValue:
		...
		return "@" + sym, nil
	case *mir.NoneValue:
		if opt, ok := backend.UnwrapNamed(v.Type()).(*typeinfo.OptionalType); ok && backend.OptionalUsesNiche(opt.Inner) {
			return llvmOptionalNoneValue(opt.Inner)
		}
		return "null", nil
	case *mir.UnaryValue:
...
func lowerGlobalComposite(state *moduleState, typ typeinfo.Type, comp *mir.CompositeValue) (string, bool, error) {
	if _, ok := typ.(*typeinfo.StringType); ok {
	...
	for _, field := range structLayout.Fields {
		...
		val, ok := items[field.Name]
		if !ok {
			typeName, err := lowerGlobalTypeName(state, field.Type)
			if err != nil {
				return "", false, err
			}
			parts = append(parts, fmt.Sprintf("%s zeroinitializer", typeName))
			offset += field.Size
			continue
		}
		var part string
		if requiresAggregateAssign(state, field.Type) {
			fieldComp, ok := val.(*mir.CompositeValue)
			if !ok {
				return "", false, nil
			}
			body, ok, err := lowerGlobalComposite(state, field.Type, fieldComp)
			if err != nil || !ok {
				return "", ok, err
			}
			...
			part = fmt.Sprintf("%s { %s }", typeName, body)
		} else {
			lit, ok, err := lowerGlobalValue(state, field.Type, val)
			if err != nil || !ok {
				return "", ok, err
			}
			irType, err := llvmBaseType(field.Type)
			...
			part = fmt.Sprintf("%s %s", irType, lit)
		}
...
func lowerGlobalStringLike(state *moduleState, comp *mir.CompositeValue) (string, bool, error) {
	ptrValue, lenValue, elems, err := becommon.SplitSliceComposite(comp)
	if err != nil {
		return "", false, err
	}
	if elems != nil {
	...
	ptrLit, ok, err := lowerGlobalValue(state, &typeinfo.RawPtrType{Inner: &typeinfo.BuiltinType{Name: "u8"}}, ptrValue)
	if err != nil || !ok {
		return "", ok, err
	}
	lenLit, ok, err := lowerGlobalValue(state, &typeinfo.BuiltinType{Name: "usize"}, lenValue)
	if err != nil || !ok {
		return "", ok, err
	}
	return fmt.Sprintf("ptr %s, %s %s", ptrLit, llvmABIIntType(), lenLit), true, nil
}

func lowerGlobalValue(state *moduleState, typ typeinfo.Type, value mir.Value) (string, bool, error) {
	switch v := value.(type) {
	case *mir.NumberValue:
		lit, err := llvmNumberLiteral(typ, v.Value)
		if err != nil {
			return "", false, err
		}
		return lit, true, nil
	case *mir.BoolValue:
		...
		return "0", true, nil
	case *mir.NameValue:
		if irType, err := llvmBaseType(typ); err != nil || irType != "ptr" {
			return "", false, err
		}
		if v.LinkName != "" {
			return "@" + becommon.SanitizeLinkName(v.LinkName), true, nil
		}
		return "@" + llvmSymbol(state, v.Path), true, nil
	case *mir.StringValue:
		if irType, err := llvmBaseType(typ); err != nil || irType != "ptr" {
			return "", false, err
		}
		return "@" + emitStringConstant(state, v.Value), true, nil
	case *mir.NoneValue:
		if opt, ok := backend.UnwrapNamed(typ).(*typeinfo.OptionalType); ok && backend.OptionalUsesNiche(opt.Inner) {
			lit, err := llvmOptionalNoneValue(opt.Inner)
			if err != nil {
				return "", false, err
			}
			return lit, true, nil
