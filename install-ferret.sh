...
DOWNLOAD_URL="$(grep -Eo "https://[^\" ]+/${ASSET}" "$RELEASE_JSON" | head -n1 || true)"
if [[ -z "$DOWNLOAD_URL" ]]; then
  echo "error: could not find release asset ${ASSET} in ${REPO} (${VERSION})" >&2
  exit 1
fi

ARCHIVE="$TMP_DIR/$ASSET"
echo "Downloading ${DOWNLOAD_URL}"
download_to "$DOWNLOAD_URL" "$ARCHIVE"
